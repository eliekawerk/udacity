...
int main() {
	...
	sprintf(in_file_name_ctr, "data/example%s/control_data.txt", example_string.c_str());

	//define file name of observations:
	sprintf(in_file_name_obs, "data/example%s/observations/", example_string.c_str());

	//read in data to measurement package list:
	helper.read_measurement_data(in_file_name_ctr, in_file_name_obs, measurement_pack_list);
	...
	for (int i = 0; i < map_1d.landmark_list.size(); i++) {
		std::cout << "ID: " << map_1d.landmark_list[i].id_i << "\t" << "value in x: " << map_1d.landmark_list[i].x_f
				<< std::endl;
	}
	std::cout << "..................................................." << std::endl;
	std::cout << "..................................................." << std::endl;
	...
	for (int i = 0; i < measurement_pack_list.size(); i++) {
		...
		std::cout << "Step " << i << " includes the move " << measurement_pack_list[i].control_s_.delta_x_f
		...
		if (measurement_pack_list[i].observation_s_.distance_f.size() < 1) {
			...
			std::cout << "	Number of Observations in current step: "
					<< measurement_pack_list[i].observation_s_.distance_f.size() << std::endl;

			for (int j = 0; j < measurement_pack_list[i].observation_s_.distance_f.size(); j++) {
				std::cout << "	Distance to a landmark: " << measurement_pack_list[i].observation_s_.distance_f[j]
						<< " m" << std::endl;
			}
		}
		std::cout << "..................................................." << std::endl;
